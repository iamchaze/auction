...
export class BidProductsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.service
      .getSingleRecord('Users', this.currentId)
      .subscribe((result) => {
        this.currentUserData = result;
      });
    this.service.getRecords('Products').subscribe((result) => {
      this.allProducts = result;
      this.currentRecord = this.allProducts.filter(
        (product: any) => product['id'] == this.currentId
      );
      ...
      this.service
        .getSingleRecord('Users', this.currentRecord.HighestBidderId)
        .subscribe((result) => {
          this.highestBidderData = result;
          this.highestBidderName = `${this.highestBidderData.firstName} ${this.highestBidderData.lastName}`;
          this.currentTime = parseInt(
            `${this.currentTime.getFullYear()}${this.currentTime.getMonth()}${this.currentTime.getDate()}`
          );
          this.auctionWindow = parseInt(
            `${this.auctionWindow.getFullYear()}${this.auctionWindow.getMonth()}${this.auctionWindow.getDate()}`
          );
  ...
  bidPriceData(value: any) {
    ...
    if (currentBid > this.productPrice) {
      ...
      this.service
        .editRecord('Products', this.currentRecord)
        .subscribe((result) => {});
      alert('Bid Submitted');
    } else {
